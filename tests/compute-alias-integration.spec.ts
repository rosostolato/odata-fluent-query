...
describe('testing compute alias integration', () => {
  it('should allow selecting computed aliases', () => {
    ...
    expect(query).toBe(
      '$select=id,name,totalPrice&$compute=price mul quantity as totalPrice',
    )
  })
  ...
  it('should allow filtering by computed aliases', () => {
    ...
    expect(query).toBe(
      '$filter=totalPrice gt 100&$compute=price mul quantity as totalPrice',
    )
  })
  ...
  it('should allow ordering by computed aliases', () => {
    ...
    expect(query).toBe(
      '$orderby=totalPrice desc&$compute=price mul quantity as totalPrice',
    )
  })
  ...
  it('should support multiple computed aliases in subsequent operations', () => {
    ...
    expect(query).toBe(
      '$filter=totalPrice gt 50&$select=id,totalPrice,shortName&$orderby=shortName&$compute=price mul quantity as totalPrice,substring(name,0,5) as shortName',
    )
  })
