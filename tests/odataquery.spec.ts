...
describe('testing ODataQuery filter by array', () => {
  ...
  test('filter by any nested', () => {
    const query = new ODataQuery<User>();
    const actual = query.filter(q => q.posts.any(p => p.comments.any(c => c.equals(null)))).toString();
  ...
  test('filter by all nested', () => {
    const query = new ODataQuery<User>();
    const actual = query.filter(q => q.posts.all(p => p.comments.all(c => c.notEquals(null)))).toString();
