...
describe('testing get_property_keys function', () => {
  ...
  const func1 = x => x.id
  test('get key in function call [1]', () => expect(get_property_keys(func1)).toStrictEqual(['id']))

  const func2 = (x: any) => x.id
  test('get key in function call [2]', () => expect(get_property_keys(func2)).toStrictEqual(['id']))

  const func3 = _x => _x.id
  test('get key in function call [3]', () => expect(get_property_keys(func3)).toStrictEqual(['id']))

  const func4 = x => x.id.biggerThan(5)
  test('get key in function call [4]', () => expect(get_property_keys(func4)).toStrictEqual(['id']))

  const func5 = x=>x.id.biggerThan(5)
  test('get key in function call [5]', () => expect(get_property_keys(func5)).toStrictEqual(['id']))

  const func6 = test => any.id.biggerThan(5)
  test('get key in function call [6]', () => expect(get_property_keys(func6)).toStrictEqual(['id']))

  const func7 = (x, y) => x.id.biggerThan(5)
  test('get key in function call [7]', () => expect(get_property_keys(func7)).toStrictEqual(['id']))

  const func8 = x => { return   x.id.biggerThan(5); }
  test('get key in function call [8]', () => expect(get_property_keys(func8)).toStrictEqual(['id']))

  const func9 = (x, y) => { y = any.description; return x.id.biggerThan(y); }
  test('get key in function call [9]', () => expect(get_property_keys(func9 as any)).toStrictEqual(['id']))

  const func10 = function (x) { return x.id.biggerThan(5); }
  test('get key in function call [10]', () => expect(get_property_keys(func10)).toStrictEqual(['id']))

  const func11 = function test(x) { return x.id.biggerThan(5); }
  test('get key in function call [11]', () => expect(get_property_keys(func11)).toStrictEqual(['id']))

  const func12 = function(x) { return x.id.biggerThan(5); }
  test('get key in function call [12]', () => expect(get_property_keys(func12)).toStrictEqual(['id']))

  const func13 = function(x){ return x.id.biggerThan(5); }
  test('get key in function call [13]', () => expect(get_property_keys(func13)).toStrictEqual(['id']))

  const func14 = function (x, y) { return x.id.biggerThan(5); }
  test('get key in function call [14]', () => expect(get_property_keys(func14 as any)).toStrictEqual(['id']))

  const func15 = x => x.id.biggerThan(5).and(x.mail.equals('mail@m.com'))
  test('get key in function call [15]', () => expect(get_property_keys(func15)).toStrictEqual(['id', 'mail']))

  const func16 = function(x) { return x.id.biggerThan(5).and(x.mail.equals('mail@m.com')); }
  test('get key in function call [16]', () => expect(get_property_keys(func16)).toStrictEqual(['id', 'mail']))
});
